...
/// Debounced background writer for session JSON + USDA artifacts.
...
impl SessionAutosaveDaemon {
    ...
    pub fn schedule(&self, dir: PathBuf, snapshot: SessionSnapshot, usda: String) {
        let _ = self.tx.send(AutosavePayload {
            dir,
            snapshot,
            usda,
        });
    ...
    pub fn flush_sync(&self, dir: &Path, snapshot: SessionSnapshot, usda: &str) -> io::Result<()> {
        write_session_files(dir, &snapshot, usda)
    }
}
        ...
        loop {
            match rx.recv_timeout(AUTOSAVE_DEBOUNCE) {
                ...
                Err(mpsc::RecvTimeoutError::Disconnected) => {
                    let _ = write_session_files(&latest.dir, &latest.snapshot, &latest.usda);
                    return;
                }
            }
        }
        let dir = if latest.dir.as_os_str().is_empty() {
            default_dir.clone()
        } else {
            latest.dir.clone()
        };
        let _ = write_session_files(&dir, &latest.snapshot, &latest.usda);
    }
}
...
pub fn write_session_files(dir: &Path, snapshot: &SessionSnapshot, usda: &str) -> io::Result<()> {
...
impl SessionAutosaveHandle {
    ...
    pub fn schedule(&self, revision: u64, snapshot: SessionSnapshot, usda: String) {
        ...
        self.daemon
            .schedule(self.dir.clone(), snapshot, usda);
    }
...
mod tests {
    ...
    fn session_snapshot_round_trips_json() {
        ...
        assert_eq!(restored.playhead_current, snapshot.playhead_current);
    ...
    fn write_and_load_session_files() {
        ...
        write_session_files(&dir, &snapshot, &usda).expect("write session");
